...
char atk_key[21][25] = {{"forward"}, {"reels"}, {"feeding"}, {"conveyor"}, {"drum"}, {"fan"}, {"redundant"}, {"grain"},
	                      {"crush"}, {"engine"}, {"entrainment"}, {"cleaning"}, {"reelh"}, {"screen"}, {"header"}, {"profilingf"},
												{"profilings"}, {"ultrasonicf"}, {"ultrasonics"}, {"latitude"}, {"longitude"}};
...
void send_atkm750_data()
{
	...
	static u8 num2 = 7;
	static u8 num3 = 2;
	const char *c = "dont know";
	USART3_Send_Data("{", strlen("{"));
	...
	for(int i=0;i<num1; i++)
	{
		...
		//sprintf(temp,"\"%s\":\" %s \",", atk_key[i], "1");
		sprintf(temp,"\"%s\":\" %s \",", atk_key[i], my_comscreen.scr_5801_result_char[j]);
		USART3_Send_Data(temp, strlen(temp));
	}
	for(int i=0;i<num2; i++)
	{
		//sprintf(temp,"\"%s\":\" %s \",", atk_key[num1+i], "2");
		sprintf(temp,"\"%s\":\" %s \",", atk_key[num1+i], my_comscreen.scr_7017_result_char[i]);
		USART3_Send_Data(temp, strlen(temp));
	}
	for(int i=0;i<num3; i++)
	{
		sprintf(temp,"\"%s\":\" %s \",", atk_key[num1+num2+i], c);
		USART3_Send_Data(temp, strlen(temp));
	}
		USART3_Send_Data("}", strlen("}"));
	//////////////////////////////////USART3改USART2///////////////////////////////
