...
{
	u8 can_mesage[3];
	u16 ADC_Value = ADC_Proc_Value(channel);//求平均值
//	ADC_Value=(ADC_Value/4096.0)*3.3;//转换成电压值
	can_mesage[2] = channel;//通道信息
	can_mesage[1] = ADC_Value >> 8;//数据
	can_mesage[0] = ADC_Value;
	CAN1_Send_Msg(can_mesage,3);//发送3个字节
//	int res=CAN1_Send_Msg(can_mesage,2);//发送1个字节
